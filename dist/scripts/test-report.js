...
async function generateTestReport() {
    ...
    const screenshotsDir = path_1.default.join(process.cwd(), 'recordings', 'test-screenshots');
    if (!fs_1.default.existsSync(screenshotsDir)) {
        logger_1.logger.error(`Screenshots directory not found: ${screenshotsDir}`);
        return;
    }
    const files = fs_1.default.readdirSync(screenshotsDir);
    // Sort files by date (newest first)
    const sortedFiles = files.sort((a, b) => {
        const statA = fs_1.default.statSync(path_1.default.join(screenshotsDir, a));
        const statB = fs_1.default.statSync(path_1.default.join(screenshotsDir, b));
        return statB.mtimeMs - statA.mtimeMs;
    });
    // Extract test info from filenames
    for (const file of sortedFiles) {
        if (file.endsWith('.png')) {
            const timestamp = new Date(fs_1.default.statSync(path_1.default.join(screenshotsDir, file)).mtime).toISOString();
    ...
    const reportPath = path_1.default.join(process.cwd(), 'reports', 'test-report.html');
  ...
  `;
    ...
    logger_1.logger.info(`Test report generated: ${reportPath}`);
    console.log(`Test report generated: ${reportPath}`);
}
generateTestReport().catch(error => {
    logger_1.logger.error(`Error generating test report: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
});
