...
function getPlatformFromUrl(url) {
    ...
    if (url.includes('cht.xxx') || url.includes('www.cht.xxx')) {
        return { key: 'Chaturbate', config: platforms_1.default['Chaturbate'] };
    }
    if (url.includes('stripchat.global') || url.includes('www.stripchat.global')) {
        return { key: 'Stripchat', config: platforms_1.default['Stripchat'] };
    }
 ...
 */
async function detectStream(url) {
    ...
    try {
        ...
        const screenshotPath = path_1.default.join(RECORDINGS_DIR, `${performerName}-check.png`);
 ...
 */
async function recordStream(streamUrl, performerName, platformKey, duration = 30) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const platformConfig = platforms_1.default[platformKey];
...
async function main() {
    try {
        ...
        if (!url) {
            ...
            Object.entries(platforms_1.default).forEach(([key, config]) => {
        ...
        if (streamInfo.isOnline && streamInfo.streamUrl && streamInfo.platform) {
            ...
            // Record the stream
            const recordingPath = await recordStream(streamInfo.streamUrl, streamInfo.performerName || 'performer', streamInfo.platform, duration);
