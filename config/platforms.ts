/**
 * Platform-specific configurations for cam sites
 */
export type PlatformConfig = {
  name: string;                             // Display name
  baseUrl: string;                          // Base URL for the platform
  identifierType: 'username' | 'channelId'; // How performers are identified
  streamUrlPattern: string;                 // Pattern for stream URLs
  isOnlineSelectors: {                      // CSS selectors to detect if performer is online
    positive: string[];                     // Elements that should exist if online
    negative?: string[];                    // Elements that should NOT exist if online
  };
  ytdlpFlags?: string[];                    // Platform-specific yt-dlp flags
  streamlinkFlags?: string[];               // Platform-specific streamlink flags
  userAgent?: string;                       // User agent to use for requests
  cookies?: boolean;                        // Whether to use cookies for auth
};
