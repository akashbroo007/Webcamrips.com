...
class StreamDetectorService {
   ...
   */
  async checkChaturbateStream(channelId: string): Promise<StreamStatus> {
    ...
    try {
      ...
      logger.error(`Error checking Chaturbate stream for ${channelId}:`, error);
      return {
        isOnline: false,
        detectedAt: new Date()
      };
   ...
   */
  async checkStripchatStream(channelId: string): Promise<StreamStatus> {
    ...
    try {
      ...
      logger.error(`Error checking Stripchat stream for ${channelId}:`, error);
      return {
        isOnline: false,
        detectedAt: new Date()
      };
   ...
   */
  async checkPerformerStatus(performer: IPerformer): Promise<Map<string, StreamStatus>> {
    const results = new Map<string, StreamStatus>();
    
    if (!performer.platforms || performer.platforms.length === 0) {
      logger.warn(`No platforms configured for performer ${performer.name}`);
      return results;
    }
      ...
      try {
        ...
        switch (platform.platform) {
          ...
          case 'Stripchat':
            ...
            break;
          default:
            logger.warn(`Platform ${platform.platform} not supported for detection`);
            status = {
              isOnline: false,
              detectedAt: new Date()
            };
        ...
        if (status.isOnline) {
          await Performer.findByIdAndUpdate(performer._id, {
            lastSeen: new Date()
          });
          
          logger.info(`Performer ${performer.name} detected online on ${platform.platform}`);
        }
      } catch (error) {
        logger.error(`Error checking status for ${performer.name} on ${platform.platform}:`, error);
   ...
   */
  async checkAllActivePerformers(): Promise<Map<string, Map<string, StreamStatus>>> {
    const performers = await Performer.find({ isActive: true });
    const results = new Map<string, Map<string, StreamStatus>>();
    
    logger.info(`Checking status for ${performers.length} active performers`);
    ...
    for (const performer of performers) {
      try {
        ...
        logger.error(`Error checking status for performer ${performer.name}:`, error);
      }
    }
...
export default new StreamDetectorService(); 